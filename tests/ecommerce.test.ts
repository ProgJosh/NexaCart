...
void test('checkout totals apply promotions and enforce cart stock', () => {
  ...
  assert.match(
    validateStock([{ productId: 'P-1008', quantity: 1 }], initialProducts) ??
      '',
    /only has 0/,
  );
});
...
void test('mock login distinguishes administrator and customer accounts', () => {
  assert.equal(resolveMockRole('ADMIN@NEXACART.TEST'), 'admin');
  assert.equal(resolveMockRole('maya@example.com'), 'customer');
});

void test('storefront formats catalog values in Philippine pesos', () => {
  assert.equal(currency(189), '₱10,962');
});

void test('admin order management updates status and generates tracking', () => {
  const updated = setOrderStatus(initialOrders, 'NX-10482', 'Shipped');
  const order = updated.find((entry) => entry.id === 'NX-10482');
  assert.equal(order?.status, 'Shipped');
  assert.match(order?.trackingNumber ?? '', /^ETR\d{9}$/);
  assert.equal(productStatus({ ...initialProducts[0], stock: 4 }), 'Low stock');
  assert.equal(
    productStatus({ ...initialProducts[0], archived: true }),
    'Archived',
  );
});
